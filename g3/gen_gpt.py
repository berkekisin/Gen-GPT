...
class GenGPT(nn.Module):
    def __init__(self, cfg: GPTConfig):
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                Block(cfg, cfg.embed_dim, cfg.num_heads, cfg.dropout, cfg.bias, edge_attn=False)
    ...
    @torch.no_grad()
    def generate(self, num_mols, num_steps, top_k, top_p=0.0):
        self_loop_index = self.num_edge_types-1
        node_token = torch.randint(0, self.num_node_target, size=(num_mols,1), device=self.cfg.device)
        degree_token = torch.zeros(size=(num_mols,1), device=self.cfg.device)
        sequence = torch.cat((node_token, degree_token), dim=-1).view(num_mols,1,-1).to(torch.int)
        adj_matrix = torch.zeros(size=(num_mols, num_steps+1, num_steps+1), device=self.cfg.device).to(torch.int)
        adj_matrix.diagonal(dim1=-2, dim2=-1).fill_(self_loop_index)
        special_token = torch.ones(size=(num_mols,1), device=self.cfg.device)
        ...
        for i in range(num_steps):
            ...
            #adj_matrix.diagonal(dim1=-2, dim2=-1).fill_(self_loop_index)
            #degree_next = adj_matrix[:,:i+2, :i+2].sum(-1) - self_loop_index
            #degree_next = torch.clamp(degree_next, max=4) # degree cant be bigger then 4 carbon is 4 
            #temp = torch.zeros_like(node_type_next, device=node_type_next.device)
            #token_next = torch.cat((node_type_next, temp), dim=-1)
            #sequence = torch.cat((sequence, token_next.unsqueeze(dim=1)), dim=1)
            #sequence[:,:,-1] = degree_next
...
if __name__ == "__main__":
    ...
    # Create an arange tensor up to the maximum element in the original tensor
  

    # Create a mask and arange for indexing
    #arange_tensor = torch.arange(1,tensor.max()).repeat(tensor.shape[0]).view(tensor.shape[0],-1)  # Get max value of tensor for the range
    #print(arange_tensor)
    #mask = arange_tensor < tensor.unsqueeze(-1)
    #print(arange_tensor[mask])
    # Use broadcasting to create a mask for the valid ranges
